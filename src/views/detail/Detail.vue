...
export default {
  ...
  methods: {
    ...
    contentScroll(position) {
      // titleTopY[index]里面都是正值
      const positionY = -position.y
      let length = this.titleTopY.length

      // positionY和titleTopY[index]里面的值进行比较
      // positionY在titleTopY[0]和titleTopY[1]之间，index = 0
      // positionY在titleTopY[1]和titleTopY[2]之间，index = 1
      // positionY在titleTopY[3]和titleTopY[3]之间，index = 2
      // positionY大于titleTopY[3]，index = 3
      // for (let i in this.titleTopY){
      // 这里的i是字符串类型 str 3 + 1 = 31
